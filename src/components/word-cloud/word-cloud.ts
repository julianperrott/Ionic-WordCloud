...
export class WordCloudComponent implements OnChanges {
    ...
    countWords (ignoreWords : string[], minWordLength : number) : any {
        return this.wordData
            ...
            .filter(function(word) {
                return ignoreWords.indexOf(word) === -1 && word.length >= minWordLength;
            })
    ...
    ngOnChanges() {
        ...
        let counts = this.countWords(ignoreWords,4);
        ...
        let scale = 15 / Math.max.apply(0, Object.keys(counts).map(key => counts[key]));
