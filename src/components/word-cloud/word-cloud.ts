...
export class WordCloudComponent implements OnChanges {
    ...
    ngOnChanges() {
        if (this.lastdata === this.wordData.data) {
            return;
        }
        ...
        this.lastdata = this.wordData.data;
        ...
        const width = this.width > this.height ? this.height : this.width;

        this.data = Object.keys(counts)
            .map(function (key) {
                return { text: key, size: counts[key] * (width / 70) };
            })
            .sort(function (a, b) {
