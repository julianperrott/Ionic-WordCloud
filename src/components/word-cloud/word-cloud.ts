...
export class WordCloudComponent implements OnChanges {
    ...
    constructor(private configurationService: ConfigurationService){
        configurationService.configurationChanged$.subscribe( 
            v => { 
                this.lastdata='';
                this.ngOnChanges();
            }
    ...
    ngOnChanges() {
        ...
        this.data = Object.keys(counts)
            .map(function (key) {
                return { text: key, size: counts[key] * (shortestAxis / ConfigurationService.settings.fontScale) };
            })
            .sort(function (a, b) {
        ...
        this.words = this.data.map(function (v) { return v.text; }).join(',');
    ...
    private populate() {
        const fontFace: string =
            ConfigurationService.settings.fontFace == null
                ? 'Impact'
                : ConfigurationService.settings.fontFace;
